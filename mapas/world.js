...
this.uiLayer.add(btnRegenIslas);
  ...
  _crearSpotPesca(hoy) {
    ...
    // Posición base: alrededor de la isla elegida con un ligero desplazamiento
    const radio = Phaser.Math.Between(70, 160);
    const ang = Phaser.Math.FloatBetween(0, Math.PI * 2);
    const x = clampX(isla.x + Math.cos(ang) * radio);
    const y = clampY(isla.y + Math.sin(ang) * radio);
